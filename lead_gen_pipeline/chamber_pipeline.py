"""End-to-end orchestration for chamber-directory processing.

Wires the parser, bulk database, and CSV loading together: load chamber URLs, parse each
directory, persist results in batches, and summarise throughput.
"""
...
class ChamberPipeline:
    ...
    def __init__(self):
        self.chamber_parser: ChamberDirectoryParser | None = None
        self.bulk_db_processor: BulkDatabaseProcessor | None = None
    ...
    async def load_chamber_urls(self, csv_file_path: Path | None = None) -> list[str]:
    ...
    async def process_single_chamber(self, chamber_url: str) -> dict[str, Any]:
        ...
        try:
            ...
            if result.success:
                ...
                logger.success(
                    f"Processed {result.chamber_info.get('name', 'Unknown')}"
                )
    ...
    async def process_chamber_batch(
    ...
    async def save_results_to_database(
        ...
        successful_results = [
            r["result"] for r in processing_results if r["success"] and r["result"]
        ]
        ...
        try:
            stats = await self.bulk_db_processor.bulk_insert_chamber_results(
                successful_results, update_existing=True
            )
            ...
            logger.success(
                f"Database now holds {db_stats.get('total_leads', 0)} records"
            )

            return {"success": True, "chamber_stats": stats, "database_stats": db_stats}

        except Exception as e:
            logger.error(f"Error saving to database: {e}")
            return {"success": False, "error": str(e)}

    async def process_all_chambers(
        self, chamber_urls: list[str] | None = None
    ) -> dict[str, Any]:
        ...
        if not chamber_urls:
            return {"success": False, "error": "No chamber URLs to process"}
        ...
        for i in range(0, len(chamber_urls), batch_size):
            ...
            logger.info(
                f"Processing batch {batch_num}/{total_batches} ({len(batch)} chambers)"
            )
            ...
            try:
                ...
                if save_result["success"]:
                    ...
                    logger.error(
                        f"Failed to save batch {batch_num}: {save_result.get('error')}"
                    )
        ...
        final_db_stats = (
            await self.bulk_db_processor.get_database_statistics()
            if self.bulk_db_processor
            else {}
        )
        ...
        summary = {
            ...
            "processing_rate_chambers_per_hour": (
                (successful_chambers / (pipeline_time / 3600))
                if pipeline_time > 0
                else 0
            ),
        ...
        logger.success(
            f"Processed {successful_chambers}/{len(chamber_urls)} chambers OK"
        )
        logger.success(
            f"   Total businesses extracted: {self.total_businesses_extracted}"
        )
        logger.success(f"   Total time: {pipeline_time:.1f} seconds")
        logger.success(
            f"   Database total records: {final_db_stats.get('total_leads', 0)}"
        )
...
async def run_chamber_pipeline(chamber_urls: list[str] | None = None) -> dict[str, Any]:
