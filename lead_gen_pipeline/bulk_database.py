"""High-throughput database operations for chamber-directory data.

Normalises LLM-extracted business records, deduplicates them by website/name, and
upserts in batches. Includes SQLite performance pragmas and helper indexes.
"""
...
class BulkDatabaseProcessor:
    ...
    def _normalize_business_data(
        self, business_data: dict[str, Any], chamber_info: dict[str, Any]
    ) -> dict[str, Any]:
        """Normalize business data for database insertion."""
        company_name = (
            business_data.get("name")
            or business_data.get("company_name")
        ...
        industry_tags = (
            [business_data["industry"]] if business_data.get("industry") else None
        )

        normalized = {
            "company_name": company_name,
            "website": website,
            "scraped_from_url": business_data.get("source_url")
            or chamber_info.get("website"),
        ...
        try:
            async with self.engine.begin() as conn:
                indexes = [
                    ...
                    "ON leads (LOWER(website))",
                    "CREATE INDEX IF NOT EXISTS idx_leads_scraped_from_url "
                    "ON leads (scraped_from_url)",
                    "CREATE INDEX IF NOT EXISTS idx_leads_created_at "
                    "ON leads (created_at)",
                    "CREATE INDEX IF NOT EXISTS idx_leads_emails_json "
                    "ON leads (json_extract(emails, '$[0]'))",
                    "CREATE INDEX IF NOT EXISTS idx_leads_phone_json "
                    "ON leads (json_extract(phone_numbers, '$[0]'))",
                ]
                ...
                for index_sql in indexes:
                    try:
                        ...
                        logger.debug(
                            f"Index skipped (already present or unsupported): {e}"
                        )
    ...
    async def bulk_insert_businesses(
        ...
        if not normalized_businesses:
            ...
            return BulkInsertStats(
                ...
                0,
                duplicates_skipped,
                len(businesses) - duplicates_skipped,
                0,
                0,
            )
        ...
        async with self.session_factory() as session:
            for i in range(0, len(normalized_businesses), self.batch_size):
                ...
                try:
                    batch_inserts, batch_updates, batch_errors = (
                        await self._process_batch(session, batch, update_existing)
                    )
        ...
        records_per_second = (
            total_processed / processing_time if processing_time > 0 else 0
        )
        ...
        logger.success(
            f"Bulk insert: {successful_inserts} inserts, {successful_updates} updates"
        )
    ...
    async def _process_batch(
        self, session: AsyncSession, batch: list[dict[str, Any]], update_existing: bool
    ) -> tuple[int, int, int]:
    ...
    async def bulk_insert_chamber_results(
        self, chamber_results: list[Any], update_existing: bool = True
        ...
        for result in chamber_results:
            ...
            try:
                stats = await self.bulk_insert_businesses(
                    result.business_listings, result.chamber_info, update_existing
                )

                all_stats[result.chamber_url] = stats
                self.total_processed += (
                    stats.successful_inserts + stats.successful_updates
                )
        ...
        logger.success("Bulk chamber processing completed:")
        ...
        try:
            async with self.session_factory() as session:
                ...
                for lead in all_leads:
                    if lead.website:
                        try:
                            ...
                            logger.debug(
                                f"Could not parse domain from {lead.website}: {e}"
                            )
                            continue
                ...
                stats = {
                    ...
                    "leads_with_emails_pct": (
                        (leads_with_emails / total_count * 100)
                        if total_count > 0
                        else 0
                    ),
                    "leads_with_phones_pct": (
                        (leads_with_phones / total_count * 100)
                        if total_count > 0
                        else 0
                    ),
                    "leads_with_websites_pct": (
                        (leads_with_websites / total_count * 100)
                        if total_count > 0
                        else 0
                    ),
                ...
                if "sqlite" in str(self.engine.url):
                    ...
                    for pragma in optimizations:
                        try:
                            ...
                            logger.debug(f"Pragma '{pragma}' skipped: {e}")
