"""Chamber-directory parser.

Coordinates the crawler and the LLM processor to walk a chamber site: find directory
pages, page through them, extract and deduplicate business listings. The crawler and LLM
processor are injectable so the full flow can be tested without a network or model.
"""
...
class ChamberDirectoryParser:
    ...
    def __init__(self, crawler=None, llm_processor=None):
        self.crawler: AsyncWebCrawler | None = crawler
    ...
    async def _fetch_page_with_retry(
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.warning(
                    f"Attempt {attempt + 1} failed for {url} (status: {status_code})"
                )
    ...
    async def _extract_chamber_info(
        ...
        try:
            ...
            return {"name": "Unknown Chamber", "website": url, "scraped_from_url": url}

    async def _find_directory_pages(self, chamber_url: str) -> list[str]:
        """Find chamber business directory pages using LLM."""
        try:
            html_content, status_code, final_url = await self._fetch_page_with_retry(
                chamber_url
            )
            ...
            directory_links = await self.llm_processor.find_directory_links(
    ...
    async def _extract_businesses_from_page(
        ...
        try:
            ...
            businesses, next_page_url = (
                await self.llm_processor.extract_business_listings(
                    html_content, final_url
                )
            )
    ...
    async def _process_directory_pagination(
        self, initial_directory_url: str
    ) -> list[dict[str, Any]]:
        """Process directory with pagination support."""
        all_businesses = []
        current_url: str | None = initial_directory_url
        pages_processed = 0
        ...
        while current_url and pages_processed < self.max_pages_per_chamber:
            ...
            page_businesses, next_page_url = await self._extract_businesses_from_page(
                current_url
            )
            ...
            if page_businesses:
                ...
                logger.info(
                    f"Added {len(page_businesses)} (total {len(all_businesses)})"
                )
            else:
        ...
        if pages_processed >= self.max_pages_per_chamber:
            logger.warning(
                f"Reached maximum pages limit ({self.max_pages_per_chamber})"
            )

        logger.success(
            f"Directory done: {len(all_businesses)} in {pages_processed} pages"
        )
    ...
    async def parse_chamber_directory(self, chamber_url: str) -> ChamberDirectoryResult:
        ...
        try:
            ...
            chamber_html, _, chamber_final_url = await self._fetch_page_with_retry(
                chamber_url
            )
            ...
            chamber_info = await self._extract_chamber_info(
                chamber_html, chamber_final_url
            )
            ...
            for directory_url in directory_links:
                ...
                try:
                    directory_businesses = await self._process_directory_pagination(
                        directory_url
                    )
                    ...
                    logger.success(
                        f"Directory {directory_url}: {len(directory_businesses)} found"
                    )
            ...
            logger.success(
                f"Chamber parsing completed: {chamber_info.get('name', 'Unknown')}"
            )
            logger.success(
                f"{len(unique_businesses)} businesses, "
                f"{total_pages_processed} pages, {processing_time:.1f}s"
            )
    ...
    def _deduplicate_businesses(
        ...
        logger.info(
            f"Deduplicated {len(businesses)} -> {len(unique_businesses)} businesses"
        )
        return unique_businesses

    async def parse_multiple_chambers(
        self, chamber_urls: list[str]
    ) -> list[ChamberDirectoryResult]:
        ...
        for i, result in enumerate(results):
            if isinstance(result, BaseException):
                logger.error(
                    f"Exception processing chamber {chamber_urls[i]}: {result}"
                )
                final_results.append(
                    ChamberDirectoryResult(
        ...
        logger.success(
            f"   Successful chambers: {successful_chambers}/{len(chamber_urls)}"
        )
